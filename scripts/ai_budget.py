...
def ensure_budget(db):
    identity = "BIGINT GENERATED BY DEFAULT AS IDENTITY" if sqlite3.is_postgres(db) else "INTEGER"
    db.execute(f"""CREATE TABLE IF NOT EXISTS ai_spend(
        id {identity} PRIMARY KEY, day TEXT NOT NULL, reserved INTEGER NOT NULL,
...
def reserve(instructions, payload):
    ...
    with closing(sqlite3.connect(DB_PATH, timeout=30)) as db, db:
        ...
        return db.execute("INSERT INTO ai_spend(day,reserved) VALUES (?,?) RETURNING id", (day,amount)).fetchone()[0]
