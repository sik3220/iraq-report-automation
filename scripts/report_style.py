...
[현식식 문체]
1. 제목에 가장 중요한 사건을 '주체, 핵심 행위/결과'로 정리한다. 핵심 대상·조건·결과가 드러나는 한 문장으로 쓰며 길이를 맞추려고 의미를 생략하지 않는다
2. summary는 제목에 없는 중요 추가 사실만 작성하며 기본은 0~1줄. 추가 정보가 없으면 빈 배열
   2줄은 예외적으로만 허용한다. 두 번째 줄을 빼면 핵심 결정·조건·피해·사업 판단이 달라질 때만 작성한다
   두 줄 중 하나가 배경 설명, 반복, 부연 수치라면 반드시 한 줄만 남긴다. 판단이 애매해도 한 줄만 작성한다
...
[회사 보고서 2026.7.30 / 8.6 / 8.13 / 9.10 문체 기준]
- 제목은 주체가 누구와 무엇을 논의·결정·지시했는지 드러내는 완결된 보고 문장
  예: 'NIC 의장, 한화 대표단과 만나 비스마야 사업 추진 현황 논의'
- 수치·수혜대상·지급조건·기한 등 사업 판단에 필요한 사실은 제목 또는 추가 내용에 보존
- 원문이 제공한 내각회의 회차가 핵심이면 '제N회 내각회의 결과'로 제목을 쓰고 결정사항을 추가 내용에 작성. 회차를 추정하지 않는다
- 단순한 '가능성 제기', '입장', '관련 동향'만으로 끝내지 말고 어떤 조건과 조치인지 구체적으로 표현
- '*'는 배경·세부 결정사항, '☞'는 당사자의 대응·반박 또는 출처가 명시된 평가에 사용
- '~했다고 발표', '~하겠다고 강조', '~할 경우 ... 경고'처럼 조건·의지를 유지하는 표현 허용
- 원문의 발언을 직인용할 때만 따옴표 사용. 요약한 문장을 직접 인용처럼 만들지 않는다
- 인명과 직책은 원문 기준. 前 직책과 현 직책을 구분하고 임의로 현직 여부를 바꾸지 않는다
- 참고 보고서의 과거 수치·테러 통계·유가·안전관리 평가는 입력 기사 근거 없이 복사 금지
- 기사 날짜는 별도로 붙이므로 title에 게시일을 중복 작성하지 않는다
...
{"status":"included|excluded|review","reason":"판단 근거",
 "title":"한국어 제목","summary":["* 가장 중요한 추가 사실"]}
"""
